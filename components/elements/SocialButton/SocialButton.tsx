...
export const SocialButton: React.FC<SocialButtonProps> = ({ provider, onClick, className }) => {
  ...
  return (
    <button
        onClick={onClick}
        className={cn(
            'flex items-center justify-center gap-3 w-full py-3.5 px-4 rounded-2xl font-semibold text-sm transition-all duration-300 active:scale-[0.98] shadow-lg hover:shadow-xl',
            config.bgColor,
            config.textColor,
            className
        )}
    >
        ...
        <span className="grow text-center">{config.name} 로그인</span>
    </button>
  );
};
